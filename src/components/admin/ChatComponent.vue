<template>
  <section class="flex flex-col w-full animate-fade-left max-h-[93dvh]">
    ...
    <div class="w-full bg-gradient-to-r to-white shadow-xl from-slate-50 via-slate-500">
      <div class="flex justify-between items-center p-4 w-full rounded-t-2xl bg-slate-100">
        <!-- Botón de regresar -->
        <div @click="toggleChatEmmit" class="flex items-center space-x-2 cursor-pointer">
          <span class="w-6 h-6 text-sky-800 transition duration-200 ease-in-out hover:text-sky-500">
            ←
          </span>
          <p class="text-base font-semibold text-sky-900">Usuarios</p>
        </div>
      ...
      <!-- Message Area -->
      <div ref="messagesElement" class="overflow-y-auto flex-1 gap-3 p-0 pt-4 pb-28 min-h-[90dvh] space-y-5 w-full bg-slate-100 max-h-dvh message-area">
...
const userChatCollection = collection(db, `condominios/${sysVals().getCondominiumId}/chats/${props.receiver}/messages/`);
...
const chatHistoryCollection = collection(db,  `condominios/${sysVals().getCondominiumId}/chats/${props.receiver}/messages/`);
...
const emmit = defineEmits(['emmit-call'])

const toggleChatEmmit = () => {
    emmit('emmit-call');
}
</script>
