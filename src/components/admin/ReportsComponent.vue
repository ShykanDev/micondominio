<template>
    <div class="flex flex-wrap gap-5 p-6 bg-white rounded-lg shadow justify-evenly">
      <ReportsCard v-for="(e, index) in complaints" :key="index"  :complaint="e.complaint" :complaint-category="e.complaintCategory" :date="e.date" :document-id="e.documentId" :resolved="e.resolved" :resolved-by="e.resolvedBy" :resolved-date="e.resolvedDate" :user-name="e.userName" :user-uid="e.userUid" />
    </div>
</template>
