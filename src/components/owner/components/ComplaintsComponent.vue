<template>
    <div v-if="showAddComplaint" class="fixed w-full max-w-md p-6 bg-white border-2 rounded-lg shadow-lg font-poppins bottom-2 left-2 border-rose-800">
        <h2 class="mb-4 text-2xl font-bold text-gray-800">Levantar una Queja</h2>
        <button @click="toggleShowAddComplaint" class="absolute px-4 py-2 text-white rounded-lg bg-slate-500 top-1 right-1 hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500">
        ...
        <form>
            <div class="mb-4">
                ...
                <input v-model="complaintCategory" type="text" id="tipo-queja" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Ingrese el tipo de queja e.g. Corte de luz">
            </div>
            <div class="mb-4">
                <label for="resumen" class="block mb-2 font-bold text-gray-700">
                    <i class="fas fa-align-left"></i> Resumen
                </label>
                <textarea v-model="complaint" id="resumen" rows="4" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Describa su queja"></textarea>
            </div>
            <div class="flex justify-end">
                <button @click.prevent="addComplaint" type="submit" class="px-4 py-2 text-white bg-blue-500 rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <i class="fas fa-paper-plane"></i> Enviar
                </button>
            </div>
        </form>
    </div>
    <button @click="toggleShowAddComplaint" class="fixed px-4 py-2 text-white border-2 rounded-lg border-slate-600 bg-rose-500 font-poppins bottom-2 right-2 hover:bg-rose-600 focus:outline-none focus:ring-2 focus:ring-rose-500">
      <i class="mr-2 fas fa-plus"></i>
      Nueva Queja
    </button>
    <div class="flex flex-wrap p-4 space-y-4 bg-white rounded-lg shadow-lg justify-evenly">
      <ComplaintCard v-for="(e, index) in complaintsList" :key="index" :complaint="e.complaint" :complaint-category="e.complaintCategory" :date="e.date" :document-id="e.documentId" :resolved="e.resolved" :resolved-by="e.resolvedBy" :resolved-date="e.resolvedDate" :user-name="e.userName" :user-uid="e.userUid"  />
